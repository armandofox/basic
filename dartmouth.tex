...
\T{IF}~\emph{expr,label1,label2,label3}.  This format is easy 
...
parsing (\T{lex} and \T{yacc} in case your compiler course was
Unix-centric).  Not so in 1964.  The simplest programming-language
parser, the 
\w[History of compiler construction]{LR parser}, wouldn't be invented
until the following year (by computer science legend Donald Knuth).  The first
\w{parser generator} was arguably written four years before in 1960, but
...
the program was run.  All
  ...
  variable names are suffixed with \$.}
...
directly into a BASIC-prompt \w{read-eval-print loop}, giving the
